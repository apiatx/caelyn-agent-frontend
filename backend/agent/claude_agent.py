    ...
    def _build_prompt(self, user_prompt: str, market_data: dict, history: list = None, is_followup: bool = False, category: str = "", chatbox_mode: bool = False, reasoning_model: str = "claude", preset_intent: str = None):
        ...
        if category == "crypto":
            ...
            crypto_preamble = (
                ...
                "DEFI & CHAIN FLOWS (DeFiLlama — on-chain protocol intelligence):\n"
                "If defillama data is present, include a 'defi_overview' section:\n"
                "- total_tvl: Global DeFi TVL in dollars — context for whether capital is flowing into DeFi\n"
                "- protocol_leaders: Top 3-5 protocols by TVL with 1d/7d TVL change — where flows are going\n"
                "- chain_leaders: Top 3-4 chains by TVL — which L1/L2 ecosystems are strongest\n"
                "- dex_context: DEX volume 24h and 1d change — whether on-chain trading is expanding/contracting\n"
                "- stablecoin_context: Total stablecoin market cap — rising = dry powder available, falling = risk-off\n"
                "Use protocol TVL changes (1d/7d) to identify where capital is rotating. A protocol gaining TVL while its token lags = potential catalyst.\n"
                "If defillama data is empty, skip this section — do NOT fabricate DeFi TVL numbers.\n\n"
                "PREDICTION MARKET PROBABILITIES (Polymarket — crypto event catalysts):\n"
                "If polymarket_crypto data is present, include relevant events in 'upcoming_catalysts' where they add genuine value:\n"
                "- Only include Polymarket events with >60% or <40% probability (high conviction, not 50/50 noise)\n"
                ...
                "5. defi_overview (DeFiLlama chain/protocol flows — include if data present)\n"
                "6. x_sentiment (ABOVE momentum picks so social signals inform the picks below)\n"
                "7. top_momentum (final conviction picks — informed by ALL data above including X sentiment)\n"
                "8. attention_signals, volume_acceleration, upcoming_catalysts (include Polymarket events here), portfolio_bias\n\n"
...
- You still have access to all the data from the original scan in the conversation history. Reference specific data points when relevant.""",
        ...
        if category == "x_trader_consensus":
            system_blocks.append({
                "type": "text",
