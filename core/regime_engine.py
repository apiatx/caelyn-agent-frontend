...
Cached for 10 minutes. Uses centralized candle fetcher with budget.
Falls back to last known regime (with reduced confidence) instead of
defaulting to neutral 0.2 when candles are unavailable.
"""
...
async def detect_market_regime(market_data_service) -> dict:
    ...
    signals = await _gather_regime_signals(market_data_service)

    failed_inputs = [k for k, v in signals.items() if v is None]
    if failed_inputs:
        print(f"[REGIME] Failed inputs: {failed_inputs}")
    if len(failed_inputs) >= 2:
        if _last_known_regime:
            degraded_confidence = max(0.3, _last_known_regime["confidence"] - 0.15)
            result = {
                "regime": _last_known_regime["regime"],
                "confidence": round(degraded_confidence, 2),
                "degraded": True,
                "reused_cache": True,
                "failed_inputs": failed_inputs,
            }
            print(f"[REGIME] {len(failed_inputs)}/5 inputs missing — reusing last regime '{result['regime']}' at confidence {result['confidence']}")
        else:
            print(f"[REGIME] {len(failed_inputs)}/5 inputs missing, no cached regime — neutral fallback")
...
async def _gather_regime_signals(svc) -> dict:
    ...
    budget = CandleBudget(max_calls=3)

    async def _fetch_spy():
        spy_bars = await svc.get_candles("SPY", days=250, budget=budget, ttl=REGIME_CANDLE_TTL)
        if spy_bars and len(spy_bars) >= 200:
            closes = [b["c"] for b in spy_bars]
            sma_200 = sum(closes[-200:]) / 200
            signals["spy_above_200dma"] = closes[-1] > sma_200

    async def _fetch_dxy():
        dxy_bars = await svc.get_candles("UUP", days=60, budget=budget, ttl=REGIME_CANDLE_TTL)
    ...
    async def _fetch_btc():
        btc_bars = await svc.get_candles("IBIT", days=250, budget=budget, ttl=REGIME_CANDLE_TTL)
    ...
    async def _fetch_vix():
        try:
            vix_data = await asyncio.to_thread(svc.fred.get_vix)
    ...
    async def _fetch_10y():
        try:
            fred_macro = await asyncio.to_thread(svc.fred.get_full_macro_dashboard)
    ...
    await asyncio.gather(
        _fetch_spy(), _fetch_dxy(), _fetch_btc(),
        _fetch_vix(), _fetch_10y(),
        return_exceptions=True,
    )

    print(f"[REGIME] candles: {budget.summary()}")
