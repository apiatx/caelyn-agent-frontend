...
- **Proxy routes** (in `routes.ts` at end): `GET /api/playbooks`, `POST /api/playbooks/score-watchlist`, `POST /api/playbooks/score-portfolio`, `POST /api/playbooks/analyze`, `POST /api/playbooks/discover`, `POST /api/playbooks/supply-chain-map` → FastAPI
