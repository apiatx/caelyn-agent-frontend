...
- **Cross-Market Scan**: When user asks about "all markets" or mentions 2+ asset classes (stocks + crypto + commodities), triggers `cross_market` category that pulls data from ALL asset classes in parallel. Enforces cross-asset ranking rules: macro regime penalty (risk-off = penalize speculative small caps), liquidity floors ($500M stocks, $100M crypto), multi-factor confluence (3/5 minimum), institutional sanity check. Prevents the system from defaulting to one asset class just because it has more signal inputs.
