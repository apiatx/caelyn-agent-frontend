...
- **Preset Intent Routing**: 13 canonical INTENT_PROFILES (daily_briefing, cross_asset_trending, microcap_asymmetry, sector_rotation, macro_outlook, earnings_catalyst, crypto_scanner, commodity_scan, social_momentum, investment_ideas, bearish_setups, thematic_scan, portfolio_review) bypass OpenAI classification entirely. Preset guard: unknown presets fall back safely to OpenAI classifier, never crash. Query refinement: when preset + query provided, `_refine_plan_with_query()` adjusts depth/style, adds asset classes, enables modules, detects tickers — without overriding base intent.
- **Heuristic Fallback**: When OpenAI classification fails (429/timeout), `_heuristic_fallback_plan()` routes based on keywords: ticker detected→deep_dive, earnings keywords→event_driven, macro→macro_outlook, crypto→single_asset_scan, sector→sector_rotation, portfolio→portfolio_review, briefing→briefing, bearish→short_setup, else→chat (lightweight, no heavy scans). Conservative default: unmatched queries go to minimal chat mode instead of full cross-asset scanning, preventing wasted API tokens on non-market queries.
- **Server-Side History Only**: Client-controlled history fields removed from QueryRequest. Conversation memory loaded exclusively server-side via `conversation_id`. Request model: `{query, preset_intent?, conversation_id?}`.
- **Structured Routing Logs**: Every request logs `[ROUTING] preset=X | query=Y | category=Z | asset_classes=[...] | modules=[...] | response_style=...` before execution.
