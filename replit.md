...
- **Best Trades Scanner**: TA-first pipeline (`get_best_trades_scan()`) runs 5 parallel Finviz screens (new highs, unusual volume, gainers, breakout, volume), fetches OHLCV via Finnhub/Polygon fallback, stacks 12+ technical signals (stage2, breakout, EMA cross, MACD, RSI, volume expansion, volatility contraction, etc), builds deterministic trade plans (entry/stop/targets/R:R from swing lows/highs), and returns structured candidates with confirmations grid. Claude polishes thesis/risk but cannot rescore or change trade plan numbers. Uses "trades" display_type with BEST_TRADES_CONTRACT enforcement.
