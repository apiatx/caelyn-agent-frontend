...
## Recent Changes
- 2026-02-16: Eliminated Polygon from ALL bulk scans (60+ call removals across 15+ methods). Polygon now only used for single-ticker research_ticker(). Switched Finviz screener from v=171 to v=111 (Overview) with header-based column parsing. Added sequential StockAnalysis enrichment with 0.8s delays, max 8 tickers. Watchlist limits Polygon to first 3 tickers with 1s delays. Added Polygon rate limiter (4 calls/min). Result: "Best Trades" scan in 41s (was 3-5 minutes).
