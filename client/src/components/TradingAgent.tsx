    ...
    <div style={{ maxWidth:1000, margin:'0 auto', fontFamily:sansFont }}>
      <div style={{ marginBottom:10 }}>
        ...
        {[
          ...
          {label: 'Trade & Investment Setups', buttons: [
          ...
          {label: 'Sectors, Themes & Alt Assets', buttons: [
          ...
          <div key={ri} style={{ display:'flex', gap:8, marginTop: ri === 0 ? 8 : 6, flexWrap:'wrap', alignItems:'center' }}>
            <span style={{ color:C.dim, fontSize:9, fontWeight:700, fontFamily:font, textTransform:'uppercase', letterSpacing:'0.06em', marginRight:4, whiteSpace:'nowrap' }}>{row.label}</span>
            {row.buttons.map(q => <button key={q.l} onClick={() => askAgent(q.p)} disabled={loading} style={{ padding:'6px 14px', background:C.card, border:`1px solid ${C.border}`, borderRadius:8, color:C.dim, fontSize:11, cursor:loading?'not-allowed':'pointer', fontFamily:font, transition:'all 0.15s' }} onMouseEnter={e => { e.currentTarget.style.borderColor = C.blue; e.currentTarget.style.color = C.bright; }} onMouseLeave={e => { e.currentTarget.style.borderColor = C.border; e.currentTarget.style.color = C.dim; }}>{q.l}</button>)}
          </div>
        ))}
        <div style={{ borderTop:`1px solid ${C.border}`, marginTop:10, paddingTop:10, display:'flex', gap:8, alignItems:'center' }}>
          <span style={{ color:C.dim, fontSize:9, fontWeight:700, fontFamily:font, textTransform:'uppercase', letterSpacing:'0.06em', marginRight:4 }}>Tools</span>
          <button onClick={() => askAgent('__PORTFOLIO__')} disabled={loading} style={{ padding:'6px 14px', background:C.card, border:`1px solid ${C.border}`, borderRadius:8, color:C.dim, fontSize:11, cursor:loading?'not-allowed':'pointer', fontFamily:font, transition:'all 0.15s' }} onMouseEnter={e => { e.currentTarget.style.borderColor = C.blue; e.currentTarget.style.color = C.bright; }} onMouseLeave={e => { e.currentTarget.style.borderColor = C.border; e.currentTarget.style.color = C.dim; }}>📋 Watchlist Review</button>
        </div>
        {showTickerInput && (
