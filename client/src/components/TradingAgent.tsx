        ...
        <div style={{ display:'flex', flexDirection:'column', gap:8, marginTop:10 }}>
          <div>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(5, 1fr)', gap:8 }}>
              ...
              ].map(q => <button key={q.l} onClick={() => askAgent(q.p)} disabled={loading} style={{ padding:'7px 4px', background:C.card, border:`1px solid ${C.border}`, borderRadius:8, color:C.dim, fontSize:11, cursor:loading?'not-allowed':'pointer', fontFamily:font, transition:'all 0.15s', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }} onMouseEnter={e => { e.currentTarget.style.borderColor = C.blue; e.currentTarget.style.color = C.bright; }} onMouseLeave={e => { e.currentTarget.style.borderColor = C.border; e.currentTarget.style.color = C.dim; }}>{q.l}</button>)}
            </div>
          </div>
          <div>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(5, 1fr)', gap:8 }}>
