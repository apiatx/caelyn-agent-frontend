        ...
        <div style={{ display:'flex', flexDirection:'column', gap:0, marginTop:10 }}>
          {[
            ...
            {label: 'Themes', buttons: [
              ...
              {l:'🪙 Crypto', p:'Full crypto market scan. Show me: global market state (total cap, BTC dominance, volume), funding rate analysis across all perps (which coins have crowded longs? which have squeeze potential from negative funding?), which crypto categories/narratives are leading (AI, memes, DeFi, L2, gaming), top momentum coins with social buzz + dev activity + funding rate data, and your highest conviction crypto trades with entry/stop/target. I want to see funding rate divergences — where price action disagrees with derivatives positioning.'},
            ]},
          ].map((row, ri) => (
            <div key={ri} style={{ marginBottom:8 }}>
              <div style={{ color:C.dim, fontSize:8, fontWeight:700, fontFamily:font, textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:4, paddingLeft:2 }}>{row.label}</div>
              <div style={{ display:'grid', gridTemplateColumns:`repeat(${row.buttons.length}, 1fr)`, gap:4 }}>
                {row.buttons.map(q => <button key={q.l} onClick={() => askAgent(q.p)} disabled={loading} style={{ padding:'5px 4px', background:C.card, border:`1px solid ${C.border}`, borderRadius:6, color:C.dim, fontSize:10, cursor:loading?'not-allowed':'pointer', fontFamily:font, transition:'all 0.15s', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }} onMouseEnter={e => { e.currentTarget.style.borderColor = C.blue; e.currentTarget.style.color = C.bright; }} onMouseLeave={e => { e.currentTarget.style.borderColor = C.border; e.currentTarget.style.color = C.dim; }}>{q.l}</button>)}
              </div>
            </div>
          ))}
          <div style={{ borderTop:`1px solid ${C.border}`, marginTop:4, paddingTop:8 }}>
            <div style={{ color:C.dim, fontSize:8, fontWeight:700, fontFamily:font, textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:4, paddingLeft:2 }}>Tools</div>
            <button onClick={() => askAgent('__PORTFOLIO__')} disabled={loading} style={{ padding:'5px 14px', background:C.card, border:`1px solid ${C.border}`, borderRadius:6, color:C.dim, fontSize:10, cursor:loading?'not-allowed':'pointer', fontFamily:font, transition:'all 0.15s' }} onMouseEnter={e => { e.currentTarget.style.borderColor = C.blue; e.currentTarget.style.color = C.bright; }} onMouseLeave={e => { e.currentTarget.style.borderColor = C.border; e.currentTarget.style.color = C.dim; }}>📋 Watchlist Review</button>
          </div>
        </div>
        {showTickerInput && (
