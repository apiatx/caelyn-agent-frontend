    ...
    def _ask_claude(self, user_prompt: str, market_data: dict, history: list = None, is_followup: bool = False, category: str = "") -> str:
        ...
        if category == "crypto":
            crypto_preamble = (
                ...
                "DOMINANCE DATA: The 'dominance' object contains btc, eth, usdt, others (all %). Include in btc_eth_summary:\n"
                "- BTC dominance % and what it signals (rising = BTC season, falling = alt season)\n"
                "- USDT dominance % — rising = risk-off (money to stables, bearish alts), falling = risk-on (bullish alts)\n"
                "- Others dominance % (altcoin share) — rising = alt season approaching\n"
                "Use the 'dominance' field in btc_eth_summary to report these numbers.\n\n"
