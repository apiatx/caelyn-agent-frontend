    ...
    def _ask_claude(self, user_prompt: str, market_data: dict, history: list = None, is_followup: bool = False, category: str = "") -> str:
        ...
        if category == "crypto":
            crypto_preamble = (
                ...
                "DATA SOURCE PRIORITY FOR MOMENTUM PICKS:\n"
                "- top_coins (CoinGecko top 50): use change_7d, change_30d + funding_rate from HyperLiquid\n"
                "- hl_additional_coins (smaller coins on HyperLiquid only): these have 24h change, funding, OI from HyperLiquid but NO 7d/30d data. Show 24h change and omit 7d rather than showing N/A. Use their funding data for squeeze/momentum analysis.\n"
                "- When recommending momentum picks, include coins from EITHER source. Squeeze candidates with extreme funding divergence are high-signal regardless of CoinGecko coverage.\n\n"
                "X/TWITTER SENTIMENT (from Grok scanning X in real-time):\n"
                "If x_sentiment data is present and non-empty, you MUST include an 'x_sentiment' section:\n"
                "- btc_sentiment: What is Crypto Twitter saying about BTC right now?\n"
                "- top_social_movers: Coins with the HIGHEST social velocity (fastest growing mentions) — potential momentum plays\n"
