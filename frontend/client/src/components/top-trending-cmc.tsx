            ...
            {trendingCoins.map((coin, index) => (
              <tr key={coin.id} className="group border-b border-white/[0.06] hover:bg-white/[0.025]" data-testid={`trending-coin-card-${coin.symbol}`}>
                <td className="px-3 py-2">
                  <a href={coinUrl(coin)} target="_blank" rel="noopener noreferrer" className="flex min-w-[150px] items-center gap-2 hover:text-amber-200">
                    ...
                    <span className="min-w-0">
                      <span className="flex items-center gap-1 truncate text-[11px] font-semibold text-gray-100">
                        {coin.name}<ExternalLink className="h-2.5 w-2.5 shrink-0 text-white/25 group-hover:text-amber-300" />
                      </span>
